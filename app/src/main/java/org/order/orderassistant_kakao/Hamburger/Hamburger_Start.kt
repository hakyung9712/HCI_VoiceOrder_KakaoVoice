...
class Hamburger_Start : AppCompatActivity() {
    //TTS
    val TAG = "Kakao"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(permission_network != PackageManager.PERMISSION_GRANTED) {
            ...
            var strText = "패티종류로는 고기, 치킨, 새우가 있습니다.패티를 골라주세요"
            //var strText="안녕"
            ttsClient?.play(strText)


            third_sttStart.setOnClickListener {
            ...
            listener1 = object : SpeechRecognizeListener {
                ...
                override fun onResults(results: Bundle?) {
                    ...
                    if (txt1 in texts.toString()) {
                        Toast.makeText(applicationContext, "고기 패티", Toast.LENGTH_LONG)
                            .show()
                        val intent =
                        ...
                        Toast.makeText(applicationContext, "치킨 패티", Toast.LENGTH_LONG)
                            .show()
                        val intent =
                            Intent(applicationContext, Hamburger_Chicken::class.java)
                        intent.putExtra("first", "포장")
                        startActivity(intent)
                        finish()
                    }else if (txt3 in texts.toString()) {
                        Toast.makeText(applicationContext, "새우 패티", Toast.LENGTH_LONG)
                            .show()
                        val intent =
